 ...
 * Created: October 31, 2022 @ 7:00PM
 * Purpose: v4: Implement outputting board to screen and file.
 */
...
vector<int> rndCrds(int);       //Generate 16 random cards of the possible 1 -54
...
//    rndCrds(16);
    ...
    Player* plyr=newPlyr(plyr1,tokens);
    board=randBrd(cards);
    plyr->board=board;
    prntPlyr(plyr); 
...
void prntBrd(const Board *b) {
    ...
    cout<<endl;
    cout<<"LOTERIA"<<setw(brdWdth-8)<<"TABLA "<<b->brdNum<<endl;
    for(int j=0;j<4;j++) {
...
void prntPlyr(Player *p) {
    ...
    cout<<"Number of Tokens: "<<p->tokens<<endl;
...
void brd2Fil(const Board *b,ofstream &file) {
    ...
    file<<endl;
    file<<"LOTERIA"<<setw(brdWdth-8)<<"TABLA "<<b->brdNum<<endl;
    for(int j=0;j<4;j++) {
        //Print out top border of board
...
Board* randBrd(Card **cards) {
    ...
    //Print out contents of vector to verify randomness
//    for(int i=0;i<16;i++) {
//        cout<<v[i]+1<<' ';
//    }
//    cout<<endl;
    ...
    for(int r=0;r<4;r++) {
        for(int c=0;c<4;c++) {
            ...
            //Print the random card that was put on the board
//            prntCrd(b->board[r][c]);
            index++;   
        }
    }    
