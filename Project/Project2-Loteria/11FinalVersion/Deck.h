...
class Deck {
    ...
    public:
        ...
        T* pick(int) const;     //Return pointer to card to the "top" of the deck
        void check(int);        //Check if the deck is empty
        T getCrd(int) const;    //Returns a card from the deck
};
...
template<class T>
T Deck<T>::getCrd(int index) const {
