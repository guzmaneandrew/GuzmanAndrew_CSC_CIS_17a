 ...
 * Created: November 5, 2022 @ 5:00 PM
 * Purpose: v8: Implement c-strings. Check for an "easy" riddle.
 */
...
void setTkns(Game &);           //Set the number of tokens for players to share
void chkBrd(string,Player *,Game &); //Checks board and updates player and game data
void chkBrd(Image,Player *,Game &);  //Checks board and updates player and game data
bool ezRddle(string,const string *); //Checks if a riddle is "easy"
char* strToC(const string);       //Returns pointer to char array
bool match(string,string);      //Uses cstring lib functions to search for string in another str
...
void dispDat(Game);                 //Displays game data
string word(int);                   //Returns word for winning pattern
...
//Game Begins
    ...
    while(!deck.cards.empty()&&gameDat.tkns>0) {
        gameDat.numRnds++;
        cout<<"------------------------------------------------------------"<<endl;
        ...
        if(input==1) {
            ...
            prntCrd(topCrd);
            //Check if riddle is easy
            if(ezRddle(topCrd.img.riddle,names)) {
                cout<<"This is an easy riddle"<<endl;
                //Send the riddle to the computer players to check their boards
                chkBrd(topCrd.img.name,plyr1Ptr,gameDat);
                chkBrd(topCrd.img.name,plyr2Ptr,gameDat);
            } else {
                //Send the riddle to the computer players to check their boards
                chkBrd(topCrd.img,plyr1Ptr,gameDat);
                chkBrd(topCrd.img,plyr2Ptr,gameDat);
            }
//            //Send the riddle to the computer players to check their boards
//            chkBrd(topCrd.img.riddle,names,plyr1Ptr,gameDat);
//            chkBrd(topCrd.img.riddle,names,plyr2Ptr,gameDat);
    ...
    for(int r=0;r<ROWS;r++) {
        for(int c=0;c<COLS;c++) {
            ...
            if(p->board->board[r][c].img.name==name) {
                cout<<p->name<<": match found at "<<r+1<<", "<<c+1<<"."<<endl; 
...
void chkBrd(Image img,Player *p,Game &gameDat) {   
    for(int r=0;r<ROWS;r++) {
        for(int c=0;c<COLS;c++) {
            //If true
                //check board for the word
            //Else if not
                //Check with riddle
            if(p->board->board[r][c].img.riddle==img.riddle) {
                cout<<p->name<<": match found at "<<r+1<<", "<<c+1<<"."<<endl; 
                p->board->board[r][c].found=true;   //set found to true
                p->plyrTkn++;   //Increment number of tokens used by player
                gameDat.tkns--;     //Decrement number of tokens available for game 
...
void dispBrd(const Player *p) {
    ...
    cout<<p->name<<endl;
    cout<<"Board "<<p->plyrTkn<<"/"<<ROWS*COLS<<" "<<endl;
    for(int r=0;r<ROWS;r++) {
        for(int c=0;c<COLS;c++) {
