...
int main(int argc, char** argv) {
    ...
    for(int record=0;record<nRecords;record++){
        Array *array=fillAry(size,record);  //Create Array structure
        //Printout the sorted array
        wrtTxt(txtFile,array,perLine);      //Write to text file
        wrtBin(binFile,array);             //Write to binary file
        destroy(array);                     //Destroy and delete the array
    }
