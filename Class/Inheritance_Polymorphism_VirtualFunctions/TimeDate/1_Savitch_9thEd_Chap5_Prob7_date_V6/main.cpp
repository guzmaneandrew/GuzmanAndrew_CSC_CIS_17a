...
int cnvrtDy(string sDay){   //convert input for days if 2 digits or 3 digits long
    if(sDay.length() == 2){
        return sDay[0] - 48; 
    } else {
...
int gtMnVal(int month, int year){
    switch(month){      //determine what day of wk it is based on month and year modifications
        case 1:
...
int gtYrVal(int year){
    return (year % 100) + (year % 100)/4;   //get what year it is for the day 
}


int gtCntry(int year){
    return 2 * (3 - ((year / 100) % 4));    //get what century it is for the day
}
